...
*/
  ...
  if (Type == "current,1h") // tomorrowIo
  {
    ...
    JsonObject forecastStep = timelines[1];
    ...
    for (byte r = 0; r < max_readings; r++) 
    {
      ...
      if (Units == "I") rain_readings[r] = WxForecast[r].Rainfall * 0.02953;   else rain_readings[r] = WxForecast[r].Rainfall;
      if (Units == "I") snow_readings[r] = WxForecast[r].Snowfall * 0.02953;   else snow_readings[r] = WxForecast[r].Rainfall;
...
*/
  ...
  if (Type == "current,1h") // tomorrowIo
  {
    ...
    JsonArray timelines = root["data"]["timelines"];
    JsonObject currentStep = timelines[0]; // fixme iterate over timelines and check whether it's current or forecast
    JsonObject forecastStep = timelines[1];
    JsonArray currentIntervals = currentStep["intervals"];
    JsonArray forecastIntervals = forecastStep["intervals"];
...
// "2019-03-20T14:09:50Z"
  ...
  if(httpCode == HTTP_CODE_OK) {
    ...
    Serial.print(json);
    if (!DecodeWeather(json, RequestType)) return false;
    client.stop();
    http.end();
    return true;
  }
